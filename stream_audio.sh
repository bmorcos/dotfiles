...
NUM=$(pactl load-module module-echo-cancel use_master_format=1 aec_method=webrtc \
      ...
      sink_master="$speakers"     sink_name=sink_main
      sink_properties=device.description=sink_main)
echo $NUM | tee $fname
...
NUM=$(pactl load-module module-remap-source master=sink_mix.monitor \
...
NUM=$(pactl load-module module-loopback latency_msec=60 adjust_time=6 source=src_ec
sink=sink_mix)
echo $NUM | tee -a $fname
